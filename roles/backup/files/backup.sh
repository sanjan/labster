...
for db in $databases; do
    if [[ "$db" != "information_schema" ]] && [[ "$db" != _* ]] ; then
        ...
        mysqldump --force --opt --user=$DBUSER --password=$DBPASSWORD --databases $db > $mysql_backupdir/$db.$(date +%Y%m%d%H%M%S).sql 2>/dev/null
        gzip -1 ${mysql_backupdir}/${db}*.sql
	cd $mysql_backupdir; ls -1tp ${db}*.gz | tail -n +8 | xargs -I {} rm -- {};
    fi
done
}
...
#perform backups
mkdir -pv $mysql_backupdir $nginx_backupdir $wordpress_backupdir
proc_mysql
proc_nginx
proc_wordpress
exit 0
